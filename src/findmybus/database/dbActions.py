...
from sqlalchemy import Engine, Sequence, delete, select
...
def upinsert(engine: Engine,
    ...
    log.info("Executing upinsert action")
...
def erase_table_entries(engine: Engine, orm_table: Base):
    log.info("Deleteing all entries from db")
    with Session(engine) as session:
        delete_stmt = delete(orm_table)
        session.execute(delete_stmt)
        session.commit()

def get_buses_position(engine: Engine,
                     line: str) -> Sequence[Positions]:
    log.info("Getting buses positions")
    with Session(engine) as session:
...
def get_bus_route(engine: Engine,
    ...
    log.info("Getting buses routes")
    with Session(engine) as session:
        return session.scalars(
            select(Routes).where(Routes.line == line)).all()
    
def get_bus_station(engine: Engine) -> Sequence[Routes]:
    log.info("Getting buses stations")
    with Session(engine) as session:
        return session.scalars(
            select(BusesStations)).all()